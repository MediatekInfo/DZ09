...
static __inline__ bhdr_t *FIND_SUITABLE_BLOCK(tlsf_t * _tlsf, int *_fl, int *_sl)
{
    u32_t _tmp = _tlsf->sl_bitmap[*_fl] & (0xFFFFFFFF << *_sl);
    bhdr_t *_b = NULL;
    ...
    if (_tmp) {
        ...
        *_fl = ls_bit(_tlsf->fl_bitmap & (0xFFFFFFFF << (*_fl + 1)));
        if (*_fl > 0) {         /* likely */
...
static char *mpool = NULL;         /* Default memory pool. */
...
/******************************************************************/
    ...
    if (tlsf->tlsf_signature == TLSF_SIGNATURE) {
        mpool = mem_pool;
        b = GET_NEXT_BLOCK(mpool, ROUNDUP_SIZE(sizeof(tlsf_t)));
...
/******************************************************************/
    ...
    TLSF_ACQUIRE_LOCK(&((tlsf_t *)mpool)->lock);

    ret = realloc_ex(ptr, size, mpool);

    TLSF_RELEASE_LOCK(&((tlsf_t *)mpool)->lock);
...
/******************************************************************/
    ...
    ret = calloc_ex(nelem, elem_size, mpool);
